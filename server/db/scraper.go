...
func CreateMapEntries(maps []MapData, shared SharedMatchData) []Map {
	match_uuid := uuid.New()
	res := make([]Map, 0)
	for _, v := range maps {
...
func FillTeamDataset(data []PlayerData, Map, Team string) []PlayerStat {
	res := make([]PlayerStat, 0)
	for _, player := range data {
		var (
			statSet  = make([]PlayerStat, 3)
...
func CreatePlayerStatEntries(data MatchDataset) ([]PlayerStat, error) {
	res := make([]PlayerStat, 0)
...
func CreateCompEntries(Maps []Map, pdata []PlayerData, data SharedMatchData) []AgentComp {
	comps := make([]AgentComp, 0)
...
func GetDbEntities(data MatchDataset) ([]Map, error) {
	maps := CreateMapEntries(data.Maps, data.Shared)
	comps := CreateCompEntries(maps, data.Players, data.Shared)
...
func CreateDbEntries(data MatchDataset, db *gorm.DB) error {
	...
	if err := db.Create(&event).Error; err != nil {
		...
		db.Find(&Event{}, "event_name = ?", data.Shared.Event).Scan(&foundEvent)
		for _, m := range maps {
